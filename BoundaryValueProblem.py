...
class BoundaryValueProblem :
    ...
    def __init__(self,f,x,s,u0,Guess,order=2) :
        ...
        if   self.order == 2 :
            ...
            raise NotImplementedError("The order of the method should be 2 or 4")
    ...
    def residual(self,u) :
        ...
        self.F[self.IndexZero,0] = u[self.IndexZero,0]-self.u0[self.IndexZero]

        self.F[self.IndexMinusOne,-1] = u[self.IndexMinusOne,-1] -self.u0[self.IndexMinusOne]

        self.F[self.IndexZero,1:] = self.diff[self.IndexZero,1:] - Derivs[self.IndexZero,1:]

        self.F[self.IndexMinusOne,:-1] = self.diff[self.IndexMinusOne,:-1] -  Derivs[self.IndexMinusOne,:-1]

        return self.F

    #%% calculate loss function for gradient descent

    def Loss(self,u) :
        return  0.5 * np.linalg.norm(self.residual(u.reshape(self.shape)))**2

    def Solver(self,u) :
        return self.residual(u.reshape(self.shape)).reshape((self.NEQ*self.NGRID))

    #%% Calculate derivative second order
    ...
    #%% Calculate derivative, fourth order
    ...
    def Solve(self) :
        ...
        # BH = basinhopping(self.Loss,guess,disp=True,niter=10,T=0.00001,niter_success=3,stepsize=0.001)
        # print BH.message
        # print "F=",BH.fun
        # return newton_krylov(self.residual,BH.x.reshape(self.shape),f_tol=1.0e-6,line_search='wolfe',verbose=True)

        # return newton_krylov(self.residual,self.Guess,f_tol=1.0e-4,line_search='wolfe',verbose=True)

        # Different alternatives, this one was the most efficient
        # guess = self.Guess.reshape((self.NEQ*self.NGRID))
        # MIN = minimize(self.Loss,guess,options={"disp":True})
        # return newton_krylov(self.residual,MIN.x.reshape(self.shape),f_tol=1.0e-4,line_search='wolfe',verbose=True)
